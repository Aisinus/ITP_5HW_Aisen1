...
vector<Order> App::readActiveOrders() {
...
void App::writeActiveOrders(vector<Order> activeOrders) {
...
Passenger App::logInPassenger(Passenger passenger) {
    validationPassengers();
...
Driver App::logInDriver(Driver passenger) {
...
Admin App::logInAdmin(Admin passenger) {
    for (int i = 0; i < validDrivers.size(); i++) {
...
bool App::logInPassengerb(Passenger passenger) {
   // validationPassengers();
    for (int i = 0; i < validPassengers.size(); i++) {
        if (passenger.login == validPassengers[i].login && passenger.password == validPassengers[i].password)
            return 1;
...
bool App::logInDriverb(Driver passenger) {
    for (int i = 0; i < validDrivers.size(); i++) {
        if (passenger.login == validDrivers[i].login && passenger.password == validDrivers[i].password)
            return 1;
    }
...
bool App::logInAdminb(Admin passenger) {
...
int App::logInPassengeri(Passenger passenger) {
    validationPassengers();
    for (int i = 0; i < validPassengers.size(); i++) {
        if (passenger.login == validPassengers[i].login && passenger.password == validPassengers[i].password)
            return i;
...
int App::logInDriveri(Driver passenger) {
    for (int i = 0; i < validDrivers.size(); i++) {
        if (passenger.login == validDrivers[i].login && passenger.password == validDrivers[i].password)
            return i;
    }
...
int App::logInAdmini(Admin passenger) {
    for (int i = 0; i < validAdmins.size(); i++) {
        if (passenger.login == validAdmins[i].login && passenger.password == validAdmins[i].password)
            return i;
    }
...
void App::acceptRideByPassenger(Passenger passenger) {
    validationPassengers();
...
void App::createOrder(Passenger passenger, string from, string to, int carType) {
    validationPassengers();
    if(logInPassenger(passenger).isBlocked)
        throw blockError("Passenger blocked");
...
void App::acceptRideByDriver(Driver driver, int numberForAccept) {
    validationDrivers();
...
void App::endOrder(Driver driver) {
    validationPassengers();
    int k = logInDriveri(driver);
...
void App::lookForOrders(Driver driver) {
    validationDrivers();
    if(logInDriver(driver).isBlocked)
        throw blockError("Driver blocked");
    activeOrders = readActiveOrders();
...
vector<string> App::lookForPayments(Passenger passenger){
    validationPassengers();
    Passenger passenger1= logInPassenger(passenger);
    return passenger1.paymentMethods;
}

void App::addPayment(Passenger passenger,string method){
    validationPassengers();
...
vector<string> App::lookForPinnedAddresses(Passenger passenger){
    validationPassengers();
    Passenger passenger1= logInPassenger(passenger);
    return passenger1.pinnedAdresses;
}

void App::addPinnedAddress(Passenger passenger,string method){
    validationPassengers();
    int k= logInPassengeri(passenger);
...
void App::writePassengersFiles(vector<Passenger> passengers) {
...
void App::writeDriversFiles(vector<Driver> passengers) {
...
void App::writeAdminFiles(vector<Admin> admins){
...
void App::blockPassenger(Admin admin, int id){
    validationPassengers();
...
void App::unBlockPassenger(Admin admin, int id){
    validationPassengers();
    if(!logInAdminb(admin))
        return;
    for(int i=0;i<validPassengers.size();i++){
...
void App::blockDriver(Admin admin, int id){
    validationDrivers();
    if(!logInAdminb(admin))
        return;
    for(int i=0;i<validDrivers.size();i++){
...
void App::unBlockDriver(Admin admin, int id){
    validationDrivers();
    if(!logInAdminb(admin))
        return;
    for(int i=0;i<validDrivers.size();i++){
...
vector<Passenger> App::seePassengers(Admin admin){
    validationPassengers();
    if(!logInAdminb(admin)){
        vector<Passenger> p;
        return p;}
...
vector<Driver> App::seeDrivers(Admin admin){
    validationDrivers();
    if(!logInAdminb(admin)){
        vector<Driver> p;
        return p;}
...
vector<Order> App::seeActiveOrders(Admin admin){
    if(!logInAdminb(admin)){
        vector<Order> p;
        return p;}
...
void App::addCarDriver(Driver driver,Car car){
    readCarQueue();
...
vector<string> App::seeCarsInQueue(Admin admin){
    vector<string> q;
    if(!logInAdminb(admin))
        return q;
    readCarQueue();
...
void App::addCarAdmin(Admin admin,int n){
    if(!logInAdminb(admin))
        return;
    readCarQueue();
    for(int i=0;i<validDrivers.size();i++){
